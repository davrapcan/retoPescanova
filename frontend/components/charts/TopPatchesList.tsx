  ...
  return (
    <div className="flex flex-col gap-2.5 w-full">
      {data.map((patch) => {
        ...
        return (
          <div key={patch.patch_id} className="flex flex-col gap-0.5">
            <div className="flex items-baseline justify-between gap-2">
              <span
                className="font-medium shrink-0"
                style={{ fontSize: '10px', color: 'var(--text-primary)', fontFamily: 'monospace' }}
              >
                {patch.bulletin_id}
              </span>
              <span
                className="truncate text-right"
                style={{ fontSize: '9px', color: 'var(--text-secondary)', flex: 1, minWidth: 0 }}
              ...
              <span
                ...
                style={{ fontSize: '10px', color: barColor }}
              >
                {affected}
              </span>
            </div>
